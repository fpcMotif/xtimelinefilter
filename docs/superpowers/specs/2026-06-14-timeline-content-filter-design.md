...
- **Date:** 2026-06-14
- **Status:** Grilled 2026-06-14 — pending user review, then writing-plans
- **Author:** f + Claude
...
Concretely, the user can, in one pass: keep only Japanese posts, hide native video, and surface only arXiv / Hacker News links — the dimensions named in the original request, generalized.
...
- **Facets (the "green-dot core"):**
  ...
  - **Link destination** (sub-facet of `link`, by outbound host): `arxiv`, `hn`, `reddit`, `youtube`, `github`, `other`.
  - **Language:** `ja` plus a small picker (`en`, `zh`, `ko`); read from the post text's `lang` attribute.
  - **Post role:** `repost` (retweet/repost via socialContext).

**Deferred to v2 (amber — heuristic or extra plumbing):**
GIF, poll, reply, thread, pinned; author `verified` / `org`; **"in one of my Lists"** (reuses Lasso's membership knowledge — attractive, but needs the Mirror/REST membership path wired in); blog/news destination heuristics. Each is listed in §11.
...
2. **Only is a whitelist, AND across families / OR within a family.** Let `F` be the set of families that contain at least one `only` criterion. If `F` is empty → **show** (subject to step 1). Otherwise `T` shows only if, for **every** family in `F`, `T` matches **at least one** of that family's `only` criteria.
3. Otherwise → **show**.

**Worked examples:**

| Active modes | A Japanese arXiv-link post | A Japanese native-video post | An English text post |
|---|---|---|---|
| `lang:ja=only` | show | show | hide |
| `lang:ja=only`, `kind:video=hide` | show | **hide** (hide wins) | hide |
| `linkDest:arxiv=only`, `linkDest:hn=only` | show | hide | hide |
| `lang:ja=only`, `linkDest:arxiv=only` | show (ja **and** arxiv) | hide (not arxiv) | hide |
...
core/link-classifier.ts  classifyHost(url) -> LinkDest           PURE, verified host table
...
- **`core/link-classifier.ts`** — `classifyHost(href): LinkDest`. A small, explicit host table (`arxiv.org`→arxiv, `news.ycombinator.com`→hn, `reddit.com`→reddit, `youtube.com`/`youtu.be`→youtube, `github.com`→github, else `other`). Pure, table-driven, trivially testable. The set of categories is data, so v2 categories are additive.
- **`core/timeline-filter.ts`** — `decide(facets, state): "show" | "hide"`. Implements §4. No DOM, no I/O. Truth-table tested.
- **`core/filter-store.ts`** — reactive `FilterState` (`Record<CriterionId, FilterMode>`) + `enabled` master signal. Persisted to `storage.sync` (single global filter in v1; per-timeline profiles deferred). Mirrors the `createSettings` pattern.
...
- **`ui/filter-bar.tsx`** — the tri-state chip bar, grouped by Family, bound to `filter-store`; shows "N hidden — show anyway" and the master toggle. Rendered in the existing open Shadow DOM tree (ADR-0003); only mounted on in-scope timelines.
...
MutationObserver (existing scanner)
   └─> filter-applier.classify(article)
          ├─ core/tweet-facets.extractFacets(article)  (uses core/link-classifier)
...
filter-bar  --(user cycles a chip)-->  filter-store.set(criterion, mode)
...
- A compact chip bar in the existing Shadow-DOM UI, grouped by Family. Each chip is tri-state with a clear visual for off/only/hide (e.g. neutral / blue-ring "only" / red-strike "hide").
...
- **Pure unit tests** (the bulk): `extractFacets` against DOM fixtures for each kind/role/lang; `classifyHost` host table; `decide` truth tables for §4 (hide-wins, AND-across-families/OR-within, empty-only, fail-open).
- **filter-store**: persistence round-trip, master toggle, default state.
...
- **Hide mechanic = reversible collapse-to-stub**, not `display:none` (Q2) — ADR-0010. Full `display:none` is a future upgrade behind a live-virtualization verification note.
- **Only-semantics = AND-across-families / OR-within-family** (Q3) — see §4 truth table.
...
### Deferred to v2
- **v2 facets:** GIF, poll, reply, thread, pinned, verified, org, blog/news destination heuristics — each needs live-DOM verification.
...
```
1. core/link-classifier.classifyHost(url)   -> verify: unit table maps arxiv/hn/reddit/youtube/github → category, else "other"
...
3. core/timeline-filter.decide(facets,state)-> verify: truth tables — hide-wins, AND-across-families/OR-within-family, empty-only=show, unclassified=show
4. core/filter-store                         -> verify: tri-state cycle off→only→hide→off; storage.sync round-trip; master toggle; default no-op
...
6. ui/filter-bar                             -> verify: tri-state chips grouped by family + "N hidden / show anyway" + master toggle; mounts only on /home and /i/lists/*
