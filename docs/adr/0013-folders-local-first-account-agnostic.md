...
## Popup Saved-row amendment — 2026-08-01

**The popup's entire collections grant is the counts summary read, and no more.** #74 gives the popup a compact Saved row — the Saved-Post total, with the live Folder count as a secondary cue — reached by one no-argument, account-free `lasso:collections` `counts` read. That single operation is pinned as the popup's *whole* collections surface: no write, no Folder read, no per-Folder membership count. A later ticket that gives the popup Destination status or a `sync now` control extends that allow-list explicitly, in the same change that extends the accepted response shape — it is never acquired by omission.

**The secret boundary this amendment records is narrower than it first sounds, and the narrow version is the true one.** The popup already reads the Convex deployment URL and device key — through its pre-existing Settings read grant, the same one its Mirror row has rendered from since ADR-0009 — and this amendment does not touch that grant or narrow it; doing so would be its own decision. What this amendment does establish is that **no `lasso:collections` operation returns a Destination secret**: the Notion and Airtable tokens a later ticket writes live under their own worker-owned key, outside `LassoSettings` entirely, so neither a collections response nor a Settings read can carry one. That key's invisibility to the popup is structural — it is not a `LassoSettings` field, so the popup's existing Settings read grant cannot reach it regardless of what that grant is later asked to return.
