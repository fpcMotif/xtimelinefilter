...
- **Device key** — the secret authorizing calls to the user's personal Convex deployment. It gates both the Membership Mirror and Folder Replica, but is never a Folder key, replica row, index, or collections response field.
...
- **Destination** — an optional, user-configured place Saved Posts are pushed to. Notion, Airtable, and future third parties are one-way and never load-bearing — a failing Destination never blocks or alters a save. **Convex Folder Replica** is the deliberate exception: it is an optional, device-key-authorized bidirectional replica of the whole account-free Folder collection, not a generic Destination read capability.
- **Collection Store** — the local Folders storage seam: one account-free contract with a durable IndexedDB implementation and a null object. It owns Folders and Saved Posts, not remote scheduling or transport; no operation on it takes an account.
- **Convex Folder Replica** — the optional personal Convex collection shared by Chrome installations configured with the same deployment URL and device key. Local IndexedDB remains the immediate offline replica; Convex orders accepted changes for convergence and never drives X.
- **Replica cursor** — the last Convex revision durably applied to one configuration-scoped local replica. A pull resumes strictly after it, so a worker restart cannot skip or replay a change as new.
- **Replica outbox** — durable, idempotent local entity changes awaiting or awaiting confirmation from Convex. It is private worker metadata and never reaches a surface response.
- **Replica tombstone** — a replicated absent/deleted entity state that prevents a stale outbox from reviving a Folder, Saved Post, membership, or bookmark evidence.
- **Replica sync state** — `local-only`, `syncing`, `current`, `offline`, `failed`, or `conflict`, rendered by the Options Folders workshop without blocking cached data.
- **Save gesture** — files the post under the cursor into a Folder. The post is the hovered one, falling back to the j/k-focused one, and the gesture climbs to the outermost post first, so a cursor inside a quoted post files the host and a repost files the underlying post. **Alt+Shift+B** files into the **Default Folder** in one worker operation, with no picker. **Alt+B** opens the **Folder Picker** so the user chooses a Folder. The per-post **Save** control on the tweet overlay binds the article directly when hover/j/k focus is awkward (e.g. status main tweet). Undo is armed only when the gesture actually wrote: re-filing a post a Folder already holds says so and arms nothing, because evicting a useful Undo to offer one that reverses nothing is worse than offering none. Bare `b` is left unbound and still reaches X's own bookmark. Live proof after rebuild/reload: `docs/research/verify-folder-save-gestures.md`.
...
- **Folder Picker** — content-script modal that lists live Folders for one captured post, marks which already hold it, and saves into the chosen Folder. Account-free; same Collections family as the workshop.
...
- **Controller** — the headless conductor for assignment, quick actions, Folder save gestures (default Folder and Folder Picker), per-post article-bound save, Undo, and fail-open Filter commands. `main.tsx` only composes dependencies.
...
- **Folder Picker controller** — cache-first Folder list for one pending capture, with holding marks and choose/close effects the Controller executes.
- **Folders workshop** — the Options section for full Folder management, replica status, and Sync now: create, rename, reorder, delete (with a disposition confirmation) and default-Folder nomination. It renders its local cache first, then refreshes a configured Convex Folder Replica and repeats its normal Folder/page reads. It remains usable with no x.com tab, no X session, no List catalog read, or no Convex connection — Folders belong to the installation's local replica, never an X account. A failed Folder or replica read leaves cached data visible with an honest error and Retry rather than an empty list.
