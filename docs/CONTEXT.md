...
- **Save gesture** — Alt+Shift+B files the post under the cursor into the **Default Folder** in one worker operation, with no picker. The post is the hovered one, falling back to the j/k-focused one, and the gesture climbs to the outermost post first, so a cursor inside a quoted post files the host and a repost files the underlying post. Undo is armed only when the gesture actually wrote: re-filing a post a Folder already holds says so and arms nothing, because evicting a useful Undo to offer one that reverses nothing is worse than offering none. Bare `b` is left unbound and still reaches X's own bookmark.
