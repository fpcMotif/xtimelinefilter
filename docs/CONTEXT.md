...
- **Storage conventions** — reactive, merged, cross-context keys go through **syncedStore** (settings, filter). One-shot reads/writes with no reactivity requirement (mirror-status publish/read, `clearLassoData`) use a raw `StorageLike` **on purpose** — don't promote them to syncedStore; the cache/merge/echo machinery buys nothing there.
