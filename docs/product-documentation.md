...
Create, rename, reorder, and delete Folders. Each row shows how many posts it holds. Click **Browse** to see the posts inside a Folder laid out like native X posts: avatar initial, @handle, full text, and embedded photos and video poster stills captured when the post was filed. Each row links to the original on X. When a Folder holds more than one page, **Load more** fetches the next batch. Press **Escape** or **Back to Folders** to return to the list. Empty Folders say so; a failed read offers Retry.
