...
> **Provenance** — batch `improve-react` · generated by the `/improve-react` skill on 2026-07-17 against commit `7ce587e` (React/Preact pass: React Doctor 0.7.8 scan + four category auditors; every finding re-verified at its cited lines). Moved from the flat `plans/013-select-click-outermost-article.md` on 2026-07-17 to separate it from the `improve` batch. Batch contract + status: [improve-react/README.md](README.md). Root index: [plans/README.md](../README.md).
