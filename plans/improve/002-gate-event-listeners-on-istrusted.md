...
> **Provenance** — batch `improve` · generated by the `/improve` skill on 2026-07-17 against commit `7ce587e` (four-agent adversarial audit; every finding re-verified against the code before planning). Moved from the flat `plans/002-gate-event-listeners-on-istrusted.md` on 2026-07-17 to separate it from the `improve-react` batch. Batch contract + status: [improve/README.md](README.md). Root index: [plans/README.md](../README.md).
...
- **Depends on**: plans/improve/001-flag-driver-synthetic-escape.md (land that first; this plan's test repairs assume the keyboard layer's flag check is unchanged)
- **Category**: security
