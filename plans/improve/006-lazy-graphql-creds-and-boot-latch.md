...
> **Provenance** — batch `improve` · generated by the `/improve` skill on 2026-07-17 against commit `7ce587e` (four-agent adversarial audit; every finding re-verified against the code before planning). Moved from the flat `plans/006-lazy-graphql-creds-and-boot-latch.md` on 2026-07-17 to separate it from the `improve-react` batch. Batch contract + status: [improve/README.md](README.md). Root index: [plans/README.md](../README.md).
