...
> **Provenance** — batch `improve` · generated by the `/improve` skill on 2026-07-17 against commit `7ce587e` (four-agent adversarial audit; every finding re-verified against the code before planning). Moved from the flat `plans/009-unmute-failure-string.md` on 2026-07-17 to separate it from the `improve-react` batch. Batch contract + status: [improve/README.md](README.md). Root index: [plans/README.md](../README.md).
...
- **Effort**: S
- **Risk**: LOW
- **Depends on**: plans/improve/005-token-scoped-toast-undo.md (same file `controller.ts`; land first to avoid a merge collision)
